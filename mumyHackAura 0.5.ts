    ...
    private attackEntity(entity: Entity) {
        ...
        if (teleport && this.settings.teleportCriticals.get()) {
            ...
            for (let offestY of [0.0626, 0, 0.0125, 0]) {
                netHandler.addToSendQueue(new C04PacketPlayerPosition(x + 0.5, y + offestY, z + 0.5, false));
            }
        }
        player.swingItem();
        ...
        for (let i = 0; i < packets; ++i) {
            ...
            if (!this.settings.returnTrip.get()) {
                const lastPos = teleportPath[teleportPath.length - 1];
                player.setPositionAndUpdate(lastPos.x + 0.5, lastPos.y + 0.0626, lastPos.z + 0.5);
            } else {
                for (let pos of mumyHackAura.FindPath.Utils.simplifyPath(teleportPath.reverse(), this.settings.packetsDistance.get())) {
                    netHandler.addToSendQueue(new C04PacketPlayerPosition(pos.x + 0.5, pos.y + 0.06256, pos.z + 0.5, spoofGround));
                }
